...
local _ui = {
    lua = {
        enable_lua = group:checkbox("\vlevi\rratan".." | \v".. euphemia.username),
    ...
    visuals = {
        ...
        watermarks = group:combobox("Watermark", {"Off", "Default", "Minimal", "Legacy", "Leviatan", "Country-Based"}),
...
for i, state in ipairs(aa_table.defensive_states) do 
    ...
    menu.pitch = group:combobox("Pitch", 'Disabled', 'Up', 'Zero', 'Random', 'Leviatan', 'Custom')
    menu.custom_pitch = group:slider("\n ", -89, 89, 0, true, "°")
...
local function downloadlogo()
    http.get("https://media.discordapp.net/attachments/882593635638599773/1162054826264367334/inter.png?ex=66c412a4&is=66c2c124&hm=7f84af8d994c6f868cf0a8cde46b673b60fd316b65d26a940277ab2fb9e0672c&", function(success, response)
        ...
        render = function()
            ...
            if _ui.visuals.inds_style.value == "Pixel" then
                if funcs.contains(_ui.visuals.inds_options.value, "Animated") then
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "leviatan  " .. euphemia.build)).x /2 + 5) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c-", 0, funcs.text_fade_animation(5, r, g, b, 255 * fraction_alpha, string.upper("leviatan  " .. euphemia.build)))
                    ...
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "leviatan  " .. euphemia.build)).x /2 + 5) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c-", 0, string.upper("leviatan  " .. euphemia.build))
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", string.upper(ctx.helps.get_state_defensive(ctx.helps.speed())))).x /2 + 2) * fraction, y + 16, r, g, b, 255 * fraction_alpha, "c-", 0, string.upper(ctx.helps.get_state_defensive(ctx.helps.speed())))
                    local m_indicators = {{text = "DT", color = is_dt == true and {r,g,b} or {92,92,92}},{text = "OS",color = is_hs == true and {r,g,b} or {92,92,92}}, {text = "QP", color = is_qp == true and {r,g,b} or {92,92,92}}, {text = "FD", color = is_fd == true and {r,g,b} or {92,92,92}}}
                    for i, v in pairs(m_indicators) do
                        r,g,b = unpack(v.color)
            ...
            if _ui.visuals.inds_style.value == "Minimal" then
                if funcs.contains(_ui.visuals.inds_options.value, "Animated") then
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "leviatan")).x /2 + 7) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c", 0, funcs.text_fade_animation(1.5, r,g,b,a * fraction_alpha, "leviatan"))
                    ...
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "leviatan")).x /2 + 7) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c", 0, "leviatan")
            ...
            if _ui.visuals.inds_style.value == "Modern" then
                if funcs.contains(_ui.visuals.inds_options.value, "Animated") then
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "leviatan")).x /2 + 7) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c", 0, to_jitter and "\a"..funcs.rgba_to_hex(r,g,b,255).."levi\aFFFFFFFFratan" or "\aFFFFFFFFlevi\a"..funcs.rgba_to_hex(r,g,b,255).."atan")
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", euphemia.build)).x /2 + 6) * fraction, y + 18, r, g, b, 255* fraction_alpha, "c", 0, funcs.text_fade_animation(1.5, r,g,b,a * fraction_alpha, euphemia.build))
                    local m_indicators = {{text = "dt", color = is_dt == true and {r,g,b} or {92,92,92}},{text = "os",color = is_hs == true and {r,g,b} or {92,92,92}}, {text = "qp", color = is_qp == true and {r,g,b} or {92,92,92}}, {text = "fd", color = is_fd == true and {r,g,b} or {92,92,92}}}
                    for i, v in pairs(m_indicators) do
                        r,g,b = unpack(v.color)
    ...
    ctx.watermark = {
        ...
        render = function()
            ...
            if funcs.contains(opts, "Desync") then
                watermark_text = to_jitter and "\a"..funcs.rgba_to_hex(r,g,b,255*fraction).."L E V I \a"..funcs.rgba_to_hex(200,200,200,230 * fraction).."A T A N" or "\a"..funcs.rgba_to_hex(200,200,200,230 * fraction).."I N T E \a"..funcs.rgba_to_hex(r,g,b,255*fraction).."R I T U S"
            else
...
client.color_log(255, 115, 105, " leviatan\0")
client.color_log(255, 255, 255, ", " .. euphemia.username)

new_notify("Welcome to \a"..funcs.rgba_to_hex(255,115,105,255).."leviatan \aFFFFFFFF"..euphemia.username, 255, 115, 105,255)
